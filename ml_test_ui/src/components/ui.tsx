...
export function Card({ children, style, className }: { children:ReactNode; style?:CSSProperties; className?:string }) {
  return <div className={className} style={{ background:'var(--s1)', border:'1px solid var(--b1)', borderRadius:'var(--r)', padding:20, ...style }}>{children}</div>
}

export function Spinner({ size=20, color='var(--accent)' }: { size?:number; color?:string }) {
  return <div style={{ width:size, height:size, border:`2px solid ${color}33`, borderTop:`2px solid ${color}`, borderRadius:'50%', animation:'spin .7s linear infinite', display:'inline-block', flexShrink:0 }} />
}
...
export function Btn({ children, onClick, disabled, variant='primary', size='md', full, style }: {
  children:ReactNode; onClick?:()=>void; disabled?:boolean
  variant?:'primary'|'ghost'|'danger'|'success'; size?:'sm'|'md'|'lg'; full?:boolean; style?:CSSProperties
}) {
  ...
  return (
    <button onClick={onClick} disabled={disabled} style={{ background:bg, color:disabled?'var(--t3)':'#fff', border:variant==='ghost'?'1px solid var(--b1)':'none', borderRadius:'var(--rs)', padding:pad, fontSize:fs, fontWeight:600, cursor:disabled?'not-allowed':'pointer', display:'inline-flex', alignItems:'center', gap:7, transition:'opacity .15s', width:full?'100%':undefined, justifyContent:full?'center':undefined, ...style }}>
      {children}
    </button>
  )
}
  ...
  return (
    <div>
      ...
      <input type={type} value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder}
        style={{ width:'100%', background:'var(--s2)', border:'1px solid var(--b1)', borderRadius:'var(--rs)', padding:'10px 14px', color:'var(--t1)', fontSize:14, outline:'none' }} />
    </div>
  )
}
...
export function ProgressBar({ pct, label, color='var(--accent)' }: { pct:number; label?:string; color?:string }) {
  return (
    <div>
      {label && <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6, fontSize:12, color:'var(--t2)' }}><span>{label}</span><span style={{ color }}>{pct}%</span></div>}
...
export function Tag({ label, color='var(--accent)', style }: { label:string; color?:string; style?: React.CSSProperties }) {
  return <span style={{ background:`${color}22`, color, border:`1px solid ${color}44`, borderRadius:6, padding:'2px 9px', fontSize:11, fontWeight:500, whiteSpace:'nowrap', ...style }}>{label}</span>
}
